...
int main(int argc, char** argv) {
    ...
    Objects objects;  // This will contain the detected objects.
    ...
    float fps = 0.0f;
    //Initialize with the locked_object_id
	int locked_object_id = 0;
        ...
        !quit)
    {
        ...
        if (grab_state == ERROR_CODE::SUCCESS) {
            ...
            // --- Set Region of Interest for Point Cloud ---
            //zed.setRegionOfInterest(objects.object_list[0].mask, { sl::MODULE::ALL });
            //cv::Mat object_mask;
            //object_mask = cv::Mat(camera_config.resolution.height, camera_config.resolution.height, CV_8SC1);
            //object_mask.setTo(0);
            //cv::Rect selection_rect;
            //selection_rect.x = 0;
            //selection_rect.y = 0;
            //selection_rect.width = 700;
            //selection_rect.height = 720;

            //cv::rectangle(object_mask, selection_rect, cv::Scalar(250), -1);

            //auto resolution = camera_config.resolution;
            //sl::Mat object_ROI(resolution, MAT_TYPE::U8_C1, object_mask.data, object_mask.step);
            //zed.setRegionOfInterest(objects.object_list[0].mask);

            //zed.setRegionOfInterest(object_ROI);
            ...
            //memset(filtered_point_cloud.getPtr<sl::float4>(MEM::CPU), 0, pc_resolution.width * pc_resolution.height * sizeof(sl::float4));
			//start the timing for the filtering process
...
//#pragma omp parallel for collapse(2)
            ...
            //saveFilteredPointCloudToPCD(filtered_point_cloud, "filtered_point_cloud.pcd");
			//filtered_point_cloud.write("filtered_point_cloud.pcd");
            // When the 's' key is pressed, save the PCD only once until it is released.
            //static bool sWasPressed = false;
            //char key1 = cv::waitKey(10);
            //if (key1 == 's') {
            //    if (!sWasPressed) {
            //        // Use your custom function to save the point cloud
            //        //saveFilteredPointCloudToPCD(filtered_point_cloud, "filtered_point_cloud.pcd");
            //        //std::cout << "PCD file saved." << std::endl;
            //        auto write_suceed = filtered_point_cloud.write("Pointcloud.ply");
			...
			//end the timing for the filtering process
